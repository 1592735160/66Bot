...
from .util import load_json, EatingEnough_List, save_json, DrinkingEnough_List, SearchLoc, FoodLoc, delete_cq_image, \
    get_cq_image_path, Meals
from .config  import  todayEat_config
...
class EatingManager:
    def __init__(self):
        ...
        self._eating_json: Path = todayEat_config.todayEat_path / "eating.json"
        self._food_json: Path = todayEat_config.todayEat_path / "food.json"
        self._greetings_json: Path = todayEat_config.todayEat_path / "greetings.json"
        self._drinks_json: Path = todayEat_config.todayEat_path / "drinks.json"
        self._img_dir: Path = todayEat_config.todayEat_path / "img"
    ...
    def get2eat(self, event: MessageEvent) -> Tuple[Message, MessageSegment]:
        ...
        if self._eating["count"][gid][uid] >= todayEat_config.eating_limit:
            ...
            # 取并集
            # 这步是为了判断获取到的食物列表和群号是否为空 但是现在在不为空的情况也判断为空了 不知道为什么
            # json中的群食物未添加 现已添加
            if len(self._eating["group_food"][gid]) > 0:
                food_list = list(set(food_list).union(set(self._eating["group_food"][gid])))
            #     拼接有问题   在获取到这个之后建议未拼接后面的食物   之前可能是类型有问题  改为messageSegment.text后可以拼接
    ...
    def todayEatFood(self, event: MessageEvent) -> Tuple[Message, MessageSegment]:
        '''
            今天主食吃什么
        '''
        # Deal with private message event FIRST
        ...
        if isinstance(event, PrivateMessageEvent):
            if len(self._eating["basic_food"]) > 0:
                ...
                return MessageSegment.text("还没有菜单呢，就先饿着肚子吧，请添加主食吧 请[添加主食 菜名]🤤")

        uid = str(event.user_id)
        gid = str(event.group_id)
        food_list: List[str] = []

        self._eating = load_json(self._food_json)
        self._init_data(gid, uid)

        # Check whether is full of stomach
        if self._eating["count"][gid][uid] >= todayEat_config.eating_limit:
            save_json(self._food_json, self._eating)
            return MessageSegment.text(random.choice(EatingEnough_List))
        else:
            # basic_food and group_food both are EMPTY
            if len(self._eating["basic_food"]) == 0 and len(self._eating["group_food"][gid]) == 0:
                return MessageSegment.text("还没有菜单呢，就先饿着肚子吧，请添加主食吧 指令为[添加主食 菜名]🤤")
            ...
            if len(self._eating["group_food"][gid]) > 0:
                food_list = list(set(food_list).union(set(self._eating["group_food"][gid])))
            #     拼接有问题   在获取到这个之后建议未拼接后面的食物   之前可能是类型有问题  改为messageSegment.text后可以拼接
            msg = Message("建议" + random.choice(food_list))
            self._eating["count"][gid][uid] += 1
            save_json(self._food_json, self._eating)
    ...
    def add_group_food(self, event: get2eat, new_food: str) -> str:
        '''
            添加至群菜单
        '''
        uid = str(event.user_id)
    ...
    def add_food_group(self, event: get2eat, new_food: str) -> str:
        '''
            添加至群菜单
        '''
        uid = str(event.user_id)
    ...
    def remove_food(self, event: get2eat, food_to_remove: str) -> str:
        '''
        ...
        self._eating = load_json(self._eating_json)
        self._init_data(gid, uid)
        status, food_fullname = self._is_food_exists(food_to_remove, SearchLoc.IN_GLOBAL,
                                                     gid)  # food_to_remove dosen't include cq
        ...
        if status == FoodLoc.IN_GROUP:
            ...
            if uid not in todayEat_config.superusers:
    ...
    def show_group_menu(self, gid: str) -> Tuple[bool, Union[Message, MessageSegment]]:
        ...
        if len(self._eating["group_food"][gid]) > 0:
            ...
            return len(self._eating["group_food"][gid]) > 1 or (
                        food_with_img > 4 and len(self._eating["group_food"][gid]) > 15), Message(msg)

        return 0, MessageSegment.text("还没有群特色菜单呢，请[添加 菜名]🤤")

    # ------------------------- food -------------------------
    def show_group_food(self, gid: str) -> Tuple[bool, Union[Message, MessageSegment]]:
        msg: str = ""
        food_with_img: int = 0
        self._eating = load_json(self._food_json)
        self._init_data(gid)
        ...
        if len(self._eating["group_food"][gid]) > 0:
            ...
            return len(self._eating["group_food"][gid]) > 20 or (
                        food_with_img > 4 and len(self._eating["group_food"][gid]) > 15), Message(msg)

        return 0, MessageSegment.text("还没有群特色主食菜单呢，请[添加 菜名]🤤")
    ...
    def show_basic_menu(self) -> Tuple[bool, Union[Message, MessageSegment]]:
        ...
        if len(self._eating["basic_food"]) > 0:
            ...
            return len(self._eating["basic_food"]) > 20 or (
                        food_with_img > 4 and len(self._eating["basic_food"]) > 15), Message(msg)
